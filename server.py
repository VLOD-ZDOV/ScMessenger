    ...
    try:
        async for raw in websocket:
            ...
            # ── Запрос контакта ──────────────────────────────
                ...
                to = msg.get("to","")
                with db_conn() as c:
                    # Получаем ключ получателя (to) чтобы отправить инициатору
                    to_row = c.execute(
                        "SELECT pubkey FROM users WHERE username=?", (to,)
                    ).fetchone()
                    # Получаем ключ инициатора (username) чтобы отправить получателю
                    my_row = c.execute(
                        "SELECT pubkey FROM users WHERE username=?", (username,)
                    ).fetchone()
                if not to_row or not my_row:
                    continue
                # Уведомляем получателя: к нему стучится username с таким ключом
                await send_to(to, {
...
async def _deliver_pending(username: str, websocket):
    ...
    for row in rows:
        try:
            ...
            # Восстанавливаем group_id если был
